...
variables <- data.frame(
  Variable = c("Especie", "Peso_gr", "Longitud1", "Longitud2", "Longitud3", 
               "Altura_cm", "Ancho_cm"),
  Tipo = c("<chr>", "<chr>", "<dbl>", "<dbl>", "<dbl>", "<dbl>", "<dbl>"),
  Tipo_dato = c("character", "character", "numeric", "numeric", "numeric", "numeric", "numeric"),
  Descripción = c("Nombre de la especie", 
                  "Peso en gramos (formato texto)", 
                  "Longitud desde la nariz hasta el comienzo de la cola (numérica)", 
                  "Longitud desde la punta de la nariz hasta la muesca de la cola. (numérica)", 
                  "Longitud desde la nariz al final de la cola (numérica)", 
...
ggplot(datos, aes(x= Especie, y = Peso_gr, fill = Especie)) + geom_boxplot() + labs(
    title = "Distribución del peso de los peces por especie",
    x = "Especie",
    y = "Peso (gramos)"
  ) +
  theme_minimal() +
...
#| output: true
#| label: figura3
#| fig-cap: "Peso individual de peces por especie"


ggplot(datos, aes(x = Especie, y = Peso_gr, color = Especie)) +
  geom_point(position = position_jitter(width = 0.15), size = 2, alpha = 0.6) +
  labs(
    title = "Peso individual de peces por especie",
    x = "Especie",
    y = "Peso (g)"
  ) +
  theme_minimal() +
...
modelo <- lm(Peso_gr ~ Longitud1 + Altura_cm + Ancho_cm + Especie, data = datos)

resultado <- tidy(modelo)

kable(resultado, digits = 3, caption = "Resumen del modelo de regresión lineal")
...
#|waring: false
#|output: false
modelo_2 <- update(modelo, .~. - Altura_cm - Ancho_cm)
...
anova_tabla <- Anova(modelo_2)
anova_df <- as.data.frame(anova_tabla)

kable(anova_df, caption = "Análisis de varianza tipo II para el modelo")
...
modelo <- aov(Peso_gr ~ Especie, data = datos)
cm <- glht(modelo, linfct = mcp(Especie = "Tukey"))
letras <- cld(cm, Letters = letters, adjust = "tukey")

letras_df_1 <- data.frame(Especie = names(letras$mcletters$Letters),
                        grupo = letras$mcletters$Letters)
media_especie <- datos %>% group_by(Especie) %>% summarize(mean = mean(Peso_gr))

datos_letras <- letras_df_1 %>% inner_join(media_especie) %>% arrange(desc(mean))
kable(datos_letras, caption = "Comparaciones múltiples de Tukey")
```
...
# Extraemos residuos y predichos
datos$r_i <- residuals(modelo_2)        # residuos
datos$s_i <- rstandard(modelo_2)        # studentizados internamente
datos$t_i <- rstudent(modelo_2)         # studentizados externamente
datos$pred <- fitted(modelo_2)          # valores predichos
...
datos$r2_i <- residuals(modelo_2)^2

# Ajustamos la regresión auxiliar
mod_aux <- lm(r2_i ~ Longitud1 + Especie, data = datos)

# Mostramos el resumen de la regresión auxiliar en tabla
kable(summary(mod_aux)$coefficients, digits = 4,
      caption = "Regresión auxiliar para el test de Breusch-Pagan")
...
# Mostramos el resultado del BP manual
...
modelo_3 <- lm(log(Peso_gr)~ log(Longitud1) + Especie, datos)


# extraemos los residuos
datos$r_i <- residuals(modelo_3)  # residuos
datos$s_i <- rstandard(modelo_3)  # studendizados INTERNAMENTE
datos$t_i <- rstudent(modelo_3)   # studentizados EXTERNAMENTE

# PRIMER PASO PARA EXPLORAR HETEROSCEDASTICIDAD (predichos vs residuos)
datos$pred <- fitted(modelo_3)
...
datos$r2_i <- residuals(modelo_3)^2

# Ajustar la regresión auxiliar para Breusch-Pagan
mod_aux <- lm(r2_i ~ log(Longitud1) + Especie, data = datos)

# Calcular el estadístico de Breusch-Pagan
...
kable(resumen_lindo, digits = 4, caption = "Resumen del modelo logarítmico")
```
...
emm <- emmeans(modelo_3, ~ Especie)

# Pasar a escala original (peso en gramos)
emm_df <- as.data.frame(emmeans::regrid(emm))

#Letras Tukey
cm_log <- glht(modelo_3, linfct = mcp(Especie = "Tukey"))
letras_log <- cld(cm_log, Letters = letters, adjust = "tukey")

letras_df <- data.frame(Especie = names(letras$mcletters$Letters),
                        grupo = letras_log$mcletters$Letters)

# Unir letras del test de Tukey
emm_df$letra <- letras_df$grupo[match(emm_df$Especie, letras_df$Especie)]

# Graficar medias ajustadas por especie
ggplot(emm_df, aes(x = Especie, y = response)) +
  geom_col(fill = "skyblue", width = 0.6) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2) +
  geom_text(aes(label = letra, y = upper.CL + 5), size = 6, fontface = "bold") +
  labs(title = "Peso medio ajustado por especie (ajustado por longitud)",
       y = "Peso estimado (g)", x = "Especie") +
...
# Sea el modelo:
modelo_3 <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = datos)
...
kbl(tabla_coef, digits = 4, caption = "Resumen de coeficientes del modelo") %>%
  kable_styling(full_width = FALSE, position = "center", latex_options = c("H", "striped"))
...
modelo_log <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = datos)

# Predicciones en escala log (lineal)
...
print("R^2 de modelo en el cual volvimos a la escala original")
r2_original
...
media_residuos <- mean(datos$t_i)
sd_residuos <- sd(datos$t_i)

ggplot(datos, aes(x = t_i)) +
...
shapiro_result <- shapiro.test(datos$t_i)
ks_result <- ks.test(datos$t_i, "pnorm", mean = mean(datos$t_i), sd = sd(datos$t_i))
jb_result <- jarque.test(datos$t_i)
#jb_result <- jarque.bera.test(datos$t_i)

# Crear tabla de resultados
tabla_normalidad <- data.frame(
  Test = c("Shapiro-Wilk", "Kolmogorov-Smirnov", "Jarque-Bera"),
  Estadístico = c(round(shapiro_result$statistic, 4),
                  round(ks_result$statistic, 4),
                  round(jb_result$statistic, 4)),
  `p-valor` = c(format.pval(shapiro_result$p.value, digits = 4, eps = 0.0001),
...
```{r}
datos$leverage <- hatvalues(modelo_3)
datos$cook <- cooks.distance(modelo_3)

# Para revisar las observaciones problemáticas
datos$es_outlier <- abs(datos$t_i) > 3
datos$es_influente <- datos$cook > 4 / nrow(datos)

# Ver casos relevantes
#subset(datos, es_outlier | es_influente)

subset(datos, es_outlier | es_influente)[, (ncol(datos)-1):ncol(datos)]
#
...
ggplot(datos, aes(x = 1:nrow(datos), y = t_i)) +
  geom_point(aes(color = es_outlier)) +
  geom_hline(yintercept = c(-3, 3), linetype = "dashed", color = "red") +
  labs(title = "Residuos Studentizados",
       x = "Índice de observación", y = "t_i (residuos studentizados)") +
  theme_minimal()
```
...
ggplot(datos, aes(x = 1:nrow(datos), y = cook)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  geom_hline(yintercept = 4 / nrow(datos), linetype = "dashed", color = "red") +
...
# Agregar residuos y predichos
datos$predichos <- fitted(modelo_3)
datos$residuos  <- residuals(modelo_3)
...
ggplot(datos, aes(x = predichos, y = residuos)) +
  ...
  xlab("Valores predichos") +
  ylab("Residuos") +
  ggtitle("Evaluación visual del supuesto de linealidad") +
  theme_minimal()
...
#| output: true
#| label: figura9
#| fig-cap: "Gráfico de residuos parciales para log(Longitud1)"

crPlot(modelo_3, variable = "log(Longitud1)")
...
# Modelo extendido con términos cuadráticos
mod_alt <- update(modelo_3, . ~ . + I(Altura_cm^2) + I(Ancho_cm^2))

# Residuos y predichos del modelo extendido
res <- residuals(mod_alt)
pred <- fitted(mod_alt)

# Gráfico de residuos del modelo extendido
...
set.seed(123)
n <- nrow(datos)
train_index <- sample(1:n, size = 0.8 * n)  # 80% entrenamiento
train <- datos[train_index, ]
test <- datos[-train_index, ]

# Ajustar modelo sobre datos de entrenamiento
modelo_entrenamiento <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = train)

# Predecir sobre test
pred_log_test <- predict(modelo_entrenamiento, newdata = test)
pred_test <- exp(pred_log_test)  # volver a la escala original

# Comparar con valores reales
real_test <- test$Peso_gr
...
datos$pred <- exp(predict(modelo_3))

ggplot(datos, aes(x = Peso_gr, y = pred)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(x = "Peso observado", y = "Peso predicho", title = "Predicción vs Real") +
  theme_minimal() +
  theme(
...
# Ver como era el modelo si quitabamos los outliers
library(moments)
library(ggplot2)
library(Metrics)
...
datos_limpios <- subset(datos, !(es_outlier | es_influente))
# 2. Ajustar modelo nuevamente sin outliers ni influyentes
modelo_3_sin_outliers <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = datos_limpios)



# QQPLOT de datos sin outliers
datos_limpios$t_i <- rstudent(modelo_3_sin_outliers)
...
ggplot(qq_data, aes(x = teoricos, y = empiricos)) +
  ...
  labs(title = "Q-Q Plot de los residuos studentizados",
       x = "Cuantiles teóricos (Normal)",
       y = "Cuantiles empíricos (residuos)")


# Distribucion de los residuos studentizados
# Media y desviación estándar de los residuos studentizados
...
# Test de normalidad para el modelo sin outliers:

# Shapiro-Wilk
shapiro.test(datos_limpios$t_i)

# Kolmogorov-Smirnov (contra normal)
ks.test(datos_limpios$t_i, "pnorm",
        mean = mean(datos_limpios$t_i),
        sd = sd(datos_limpios$t_i))
...
# EVALUAR EL RENDIMIENTO PREDICTIVO DEL MODELO SIN OUTLIERS:
...
datos_train <- datos_limpios[indices_train, ]
datos_test  <- datos_limpios[-indices_train, ]

modelo_train <- lm(log(Peso_gr) ~ log(Longitud1) + Especie, data = datos_train)

pred_test <- predict(modelo_train, newdata = datos_test)
real_test <- log(datos_test$Peso_gr)  # valor real transformado



# Metricas de evaluacion de la capacidad predictiva
rmse_val <- rmse(real_test, pred_test)
mae_val  <- mae(real_test, pred_test)
r2_val   <- 1 - sum((real_test - pred_test)^2) / sum((real_test - mean(real_test))^2)
...
ggplot(data.frame(real = real_test, pred = pred_test), aes(x = real, y = pred)) +
  ...
  labs(title = "Predicción vs Real (Conjunto de test)",
       x = "log(Peso_gr) real",
       y = "log(Peso_gr) predicho") +
  theme_minimal()
