...
diseno_clusters <- svydesign(id = ~cluster_id, data = muestra_conglomerados,  fpc = rep(20, nrow(muestra_conglomerados)) # 20 es el total de clusters en la población
)
...
res_media <- svymean(~balance, diseno_dos_etapas)
...
razon_campana <- svyratio(~balance, ~duration, diseno_dos_etapas)
print(razon_campana)
```
...
diseno_jk <- as.svrepdesign(diseno_dos_etapas, type = "JK1")
...
diseno_boot <- as.svrepdesign(diseno_dos_etapas, type = "bootstrap", replicates = 500)
...
diseno_calibrado <- calibrate(design = diseno_dos_etapas, 
                              formula = ~age, 
...
summary(weights(diseno_dos_etapas))
...
muestra_2etapas$peso_original <- weights(diseno_dos_etapas)
...
modelo_greg <- svyglm(balance ~ age, design = diseno_dos_etapas)
...
res_antes <- svymean(~balance, diseno_dos_etapas)
...
diseno_terciario <- subset(diseno_dos_etapas, education == "tertiary")
...
print(res_dominio)
print(svymean(~balance, diseno_dos_etapas))
```
