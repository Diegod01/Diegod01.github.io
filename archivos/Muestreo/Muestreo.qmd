...
# --- 0. Preparación Robusta ---
# Es vital que 'N' sea exactamente el número de filas de la data que entra al diseño
bank_clean <- bank_data %>% filter(balance > 0) %>% drop_na(balance, job, education)
N <- nrow(bank_clean)
n_target <- 500

# Función auxiliar para extraer métricas sin errores
get_metrics <- function(res, name) {
  data.frame(
    Diseño = name,
    Media = as.numeric(coef(res)),
    SE = as.numeric(SE(res)),
    Deff = as.numeric(deff(res))
  )
}

# --- 1. Ejecución de Diseños ---

# 1. MAS sin reemplazo (Referencia)
set.seed(123)
m_mas_sr <- bank_clean[sample(N, n_target), ]
d_mas_sr <- svydesign(id=~1, fpc=rep(N, n_target), data=m_mas_sr)
r1 <- get_metrics(svymean(~balance, d_mas_sr, deff=TRUE), "MAS SR")

# 2. Bernoulli
p_b <- n_target/N
m_bern <- bank_clean[rbinom(N, 1, p_b) == 1, ]
d_bern <- svydesign(id=~1, probs=rep(p_b, nrow(m_bern)), data=m_bern)
r2 <- get_metrics(svymean(~balance, d_bern, deff=TRUE), "Bernoulli")

# 3. MAS con reemplazo
m_mas_cr <- bank_clean[sample(N, n_target, replace=TRUE), ]
d_mas_cr <- svydesign(id=~1, weights=rep(N/n_target, n_target), data=m_mas_cr)
r3 <- get_metrics(svymean(~balance, d_mas_cr, deff=TRUE), "MAS CR")

# 4. Estratificado (Job)
m_est <- bank_clean %>% group_by(job) %>% sample_frac(n_target/N) %>% ungroup()
N_h <- bank_clean %>% group_by(job) %>% summarize(N_h = n())
m_est <- m_est %>% left_join(N_h, by="job")
d_est <- svydesign(id=~1, strata=~job, fpc=~N_h, data=m_est)
r4 <- get_metrics(svymean(~balance, d_est, deff=TRUE), "Estratificado")

# 5. Sistemático (Corregido para evitar error de dims)
k <- floor(N/n_target)
idx_sis <- seq(sample(1:k, 1), N, by=k)
m_sis <- bank_clean[idx_sis, ]
d_sis <- svydesign(id=~1, fpc=rep(N, nrow(m_sis)), data=m_sis)
r5 <- get_metrics(svymean(~balance, d_sis, deff=TRUE), "Sistemático")

# 6. Poisson
pi_p <- pmin((bank_clean$age / sum(bank_clean$age)) * n_target, 1)
sel_p <- rbinom(N, 1, pi_p) == 1
d_poi <- svydesign(id=~1, probs=pi_p[sel_p], data=bank_clean[sel_p,])
r6 <- get_metrics(svymean(~balance, d_poi, deff=TRUE), "Poisson")

# 7. PPS (Con reemplazo)
p_pps <- bank_clean$balance / sum(bank_clean$balance)
idx_pps <- sample(N, n_target, replace=TRUE, prob=p_pps)
d_pps <- svydesign(id=~1, probs=p_pps[idx_pps], data=bank_clean[idx_pps,])
r7 <- get_metrics(svymean(~balance, d_pps, deff=TRUE), "PPS")

# 8. PI-PS (Sin reemplazo - Brewer)
pi_pips <- pmin(n_target * (bank_clean$balance / sum(bank_clean$balance)), 1)
# Usamos una muestra más pequeña para pi_ps si el algoritmo falla por tamaño
d_pips <- svydesign(id=~1, probs=pi_pips, data=bank_clean, pps="brewer")
r8 <- get_metrics(svymean(~balance, d_pips, deff=TRUE), "PI-PS")

# 9. Conglomerados (Education)
d_cong <- svydesign(id=~education, data=bank_clean)
r9 <- get_metrics(svymean(~balance, d_cong, deff=TRUE), "Conglomerados")

# 10. Dos Etapas (UPM: Job)
m_2et <- bank_clean %>% group_by(job) %>% sample_frac(0.1) %>% ungroup()
d_2et <- svydesign(id=~job + 1, data=m_2et)
r10 <- get_metrics(svymean(~balance, d_2et, deff=TRUE), "Dos Etapas")

# --- 2. Tabla de Resultados ---
tabla_final <- bind_rows(r1, r2, r3, r4, r5, r6, r7, r8, r9, r10) %>% arrange(Deff)
print(tabla_final)
